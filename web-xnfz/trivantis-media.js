...
function ObjMediaBuild() 
{
  ...
  else
  {
	  if(this.isVideo())
	  {
		  ...
		  // zzl 解决安卓新版QQ浏览器以实际宽高展示播放器
			var data = is.isMobile.Android();
			var zoom = '';
			if (data && data.input && data.input.indexOf('MQQBrowser') !== -1) {
				var t_data = getFillBrowserScale();
				zoom = 'zoom:' + t_data.scale + ';'
			}
		  //LD-3943 -- Updated
		 //LD-5928 force transform scale to 1 for video as it causes issues with rollover when it is above 1 - the size of mediaElement is the one that is respected anyway
	      this.divInt += '<video style=transform:scale(1);'+ zoom +' width=\'' + this.w + 'px\' height=\'' + this.h + 'px\' ' + (this.bControl ? "controls=\"controls\"" : "") + ' id=\'' + this.name + 'Media\' name=\'' + this.name + 'Media\' ' + ((this.bAutoStart && this.bAllowAutoStart) ? "autoplay=\"autoplay\"" : "") + (is.iOS?' playsinline':' webkit-playsinline')+(this.altName ?  ' alt="' + this.altName + '"' : "") +    '>';
