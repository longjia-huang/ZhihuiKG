...
function ObjLayer(id,pref,frame, divEle) {
  ...
  if(this.styObj && !bInherit) {
	  ...
	  if (this.ele && this.ele.id) {
	  	var iframe = document.getElementById(this.ele.id+"iframe") || getHTMLEleByID(this.ele.id+"iframe");
	  	if (iframe) {
			var display = iframe.getAttribute('display') || '';
			if (display) {
				iframe.removeAttribute('display');
				iframe.style.display = display;
			} else {
				iframe.setAttribute('display', '' + (iframe.style.display || ''));
...
function getOrientation(){
	if( getDisplayWindow() && window.myTop ){
		try{
			...
			{
				// zzl 修正横竖屏判断
				var $iframe = window.myTop.document.getElementById('content_frame');
				if ($iframe) {
					return $iframe.clientHeight > $iframe.clientWidth ? "portrait" : "landscape";
				} else {
					// 原代码
...
{	// zzl  1009
	if(is.ie8 || !window.bTrivResponsive) return 1920;
...
function adjustForFixedPositon(obj, respChange, biOSBarChange){
	...
	if(obj.bFixedPosition && obj.bBottom){
		...
		if(isSppInIosFrame()){
			...
			var scrollEle = frameParent.nodeName == "DIV" && frameParent.scrollHeight > frameParent.offsetHeight ? frameParent : getDisplayWindow();
...
function getFillBrowserScale()
{
	...
	// zzl 缓存缩放比例
	if (typeof scaleFill === 'number' && scaleFill !== Infinity) {
		window.sessionStorage.setItem('scormScaleFill', JSON.stringify({ scaleFill: scaleFill }))
	} else {
		var data = window.sessionStorage.getItem('scormScaleFill');
		if (data) {
			try {
				scaleFill = JSON.parse(data).scaleFill
			} catch (e) {
				console.log(e)
			}
		}
...
function getAncestorFrameOrIFrame(win, tag, cnt)
{
	...
	cnt = cnt || 0;
	// zzl 修复oss可能导致跨域问题
	if (win.frameElement && win.frameElement.id && ['content_frame', 'content-frame', 'contentFrame'].indexOf(win.frameElement.id) !== -1 && cnt < 10)
		return win.frameElement;
	else if (win.frameElement && win.frameElement.tagName.toLowerCase() == tag && win.frameElement.parentElement.id != 'trivTitleMgrFrameset')
		return win.frameElement;
...
function trivScrollTo( wind , objToScroll )
{
	...
	if(typeof(objToScroll) !== 'undefined')
	{
		...
		if(objToScroll === 'top') {
			...
			// zzl 过滤获取不到节点的情况
      var obj = getDisplayDocument().getElementById(objToScroll);
      if (obj) {
        win.scrollTo(0 , getTopofObj(obj));
      }
		}
	}
...
// zzl 兼容调用该函数的错误
if (typeof window.updateFLVPlayer !== 'function') {
	window.updateFLVPlayer = function () {}
}
