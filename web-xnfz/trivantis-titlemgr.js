...
TMPr.loadTest = function( fn, tn, pn )
{
  ...
  if( !this.arTests[tIdx].bLoaded )
  {
    ...
    if( !httpReq.responseText || httpReq.responseText.length == 0 )
    {
      if( window.console ) console.log('Offline mode: test data "' + xfn + '" not loaded. Use a local web server for full functionality.');
      return;
    }
